...
(function(){
  ...
  function injectUI() {
    ...
    <div class="container">
      ...
      <div class="search-box">
        <input type="text" id="input" placeholder="Wpisz domenę (.sus, .xdd...)">
        <button id="go">➡</button>
      </div>
    ...
    go && go.addEventListener('click', () => {
      ...
      if (!q) return;
      const url = 'https://resolver.odns.dev/?domain=' + encodeURIComponent(q);
  ...
  chrome.storage.local.get(['odns_enabled'], function(res){
    ...
    if (!enabled) {
      try { window.location.replace(BLANK); } catch (e) { window.location.href = BLANK; }
      return;
    }
    injectUI();
  });
